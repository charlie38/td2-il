...
class Graphe {
	...
	public static void toDot(Graphe G, String filename) throws IOException {
		...
		DataOutputStream out = new DataOutputStream(fich)) {
			...
			for (Iterator I = G.treeSet.iterator(); I.hasNext();) {
				Node N = (Node) I.next();
				nodes += " " + N.toDot();
				TreeSet succ = N.succ();
	...
	private void parcoursLargeur(Node S, boolean visu) {
		...
		while (!F.isEmpty()) {
			...
			for (Iterator I = N.succ().iterator(); I.hasNext();) {
				Arc A = (Arc) I.next();
				Node N2 = (Node) A.to();
				if ((N2.color().equals(BLACK)) && (!F.contains(N2))) {
						F.add(N2);
						if (visu) {A.setColor("blue");}
				}
			}
			F.remove(N);
			N.setColor("blue");
